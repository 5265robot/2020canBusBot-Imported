...
public class RobotContainer {
  ...
  private final IntakeSubsystem m_intake =
    new IntakeSubsystem();
  private final ArmPIDSubsystem m_armPID =
    new ArmPIDSubsystem();

  // drn --- declaring an instance of the XBox controller
  private final XboxController m_xboxController = 
    new XboxController(OIConstants.kDriverControllerPort);
  private final Joystick m_thrustMaster =
    new Joystick(OIConstants.kTrustMasterPort);
  ...
  private final Command m_simpleDriveForward = new RunCommand(
    () -> m_robotDrive.arcadeDrive(AutoConstants.kPower,0.0), m_robotDrive)
    .withTimeout(AutoConstants.kTimeOut);

  private final Command m_simpleDriveReverse = new StartEndCommand(
    () -> m_robotDrive.arcadeDrive(-AutoConstants.kPower,0.0), 
  ...
  public RobotContainer() {
    ...
    sbConfig.add(m_chooser)
      .withSize(3, 1)
      .withPosition(0, 0);

    // drn -- put power onto shuffleboard
    sbConfig.add("PDP voltage", pdp.getVoltage())
      .withSize(1, 1)
      .withPosition(8, 0);
    // drn -- put camera on shuffleboard
    sbConfig.add(camera01)
      .withSize(6, 5)
      .withPosition(2, 0);
  ...
  private void configureButtonBindings() {
    ...
    final JoystickButton intakeButton =
      new JoystickButton(m_xboxController,Constants.kIntakeButton);
    final JoystickButton intakeReverse = 
      new JoystickButton(m_xboxController, Constants.kIntakeReverseButton);
    intakeButton.whenPressed(() ->
      m_intake.intakeOn(IntakeConstants.kIntakePower, Constants.currentIntakeState));
    intakeReverse.whenPressed(() ->
      m_intake.intakeOn(-IntakeConstants.kIntakePower, Constants.currentIntakeState));
      
    // conveyor
    final JoystickButton conveyorButton =
      new JoystickButton(m_xboxController, Constants.kConveyorPulseButton);
    conveyorButton.whileHeld(new RunCommand(() ->
      m_intake.upperAndLowerOn(IntakeConstants.kConveyorPulsePower))
      .withTimeout(IntakeConstants.kConveyorOneBallPulse)
      .andThen(new RunCommand(() ->
      m_intake.upperAndLowerOff())));

    final JoystickButton conveyorFiveBallEmpty = 
      new JoystickButton(m_xboxController, Constants.kConveyorEmptyButton);
    conveyorFiveBallEmpty.whenPressed(() ->
      m_intake.upperAndLowerOn(IntakeConstants.kConveyorFullPower))
      .whenReleased(() -> m_intake.upperAndLowerOff());
    
    final JoystickButton conveyorThrustButton =
      new JoystickButton(m_thrustMaster, Constants.kThrustPulseButton);
    conveyorThrustButton.whileHeld(new RunCommand(() ->
      m_intake.upperAndLowerOn(IntakeConstants.kConveyorPulsePower))
      .withTimeout(IntakeConstants.kConveyorOneBallPulse)
      .andThen(new RunCommand(() ->
      m_intake.upperAndLowerOff())));
    ...
    testMotorButton.whenHeld(new RunCommand(() ->
      m_miscDrive.misc01Drive(MiscConstants.kmisc01Power))
      .withTimeout(0.3)
      .andThen(new RunCommand(() ->
        m_miscDrive.misc01Drive(0.0))));
    ...
    // camera
    final JoystickButton switchCamera =
      new JoystickButton(m_xboxController, Constants.kSwitchCameraButton);
    switchCamera.whenPressed(
      () -> cameraSwitch());
    ...
    climbArmUp.whenPressed(
      new InstantCommand(m_armPID::enable, m_armPID));
    //D-Pad RIGHT
    final POVButton climbArmExtend =    // extends elbow
      new POVButton(m_xboxController, OIConstants.kClimbArmExtend);
    climbArmExtend.whenPressed(
      () -> m_armPID.fireElbow(), m_armPID);
    // release arm and webbing
    final JoystickButton releaseArmAndWebbing =
      new JoystickButton(m_xboxController, Constants.kElbowExtend);
    releaseArmAndWebbing.whenPressed(
      () -> m_armPID.firePinch()); 
    //D-Pad DOWN
    final POVButton winchOn =           // toggle that turns the winch on
      new POVButton(m_xboxController, OIConstants.kWinchOnToggle);
    winchOn.whenPressed(
      () -> m_armPID.winchOn(ArmConstants.kWinchPower));
      //D-Pad LEFT
    final POVButton climbArmDown =      // toggle that drops the arm
      new POVButton(m_xboxController, OIConstants.kClimbArmDownToggle);
    climbArmDown.whenPressed(
      new InstantCommand(m_armPID::armDown, m_armPID)
      .withTimeout(0.5));
    // kX button
    final JoystickButton climbArmOff =  // stops PID, resets
      new JoystickButton(m_xboxController, Constants.kClimbOffButton);
    climbArmOff.whenPressed(
      () -> m_armPID.testArmOff());
      
    // speed
    final JoystickButton slowDown =
      new JoystickButton(m_xboxController, Constants.kSlowDown);
    slowDown.whenPressed(() ->
      m_robotDrive.halfPower());
